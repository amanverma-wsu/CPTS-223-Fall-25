...
struct PQNode
{
    ...
    // Comparator for min-heap (greater distance = lower priority)
...
public:
    ...
    // Uses a priority queue (min-heap) to efficiently select the next closest unvisited city
    map<string, int> dijkstraShortestPath(const string &start)
    {
        ...
        // Priority queue (min-heap) to store cities with their current distances
        // Using greater<PQNode> creates a min-heap
        priority_queue<PQNode, vector<PQNode>, greater<PQNode>> pq;
        pq.push(PQNode(start, 0));
